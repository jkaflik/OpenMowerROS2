 ...
 */
        ...
        public:
            KALMAN_VECTOR(OrientationMeasurement2, T,
            2)
 ...
 */
        ...
        class OrientationMeasurementModel2 : public Kalman::LinearizedMeasurementModel<State < T>,
                                             OrientationMeasurement2<T>,
                                             CovarianceBase

        > {
        public:
        //! State type shortcut definition
        typedef OpenMower::GPSOrientation::State<T> S;

        //! Measurement type shortcut definition
        typedef OpenMower::GPSOrientation::OrientationMeasurement2<T> M;
        ...
        M h(const S &x) const {
            ...
            measurement.vx() = x.vx() * std::cos(x.theta()) - (std::sin(x.theta()) * antenna_offset_x * x.vr()) -
                               (std::cos(x.theta()) * antenna_offset_y * x.vr());
            measurement.vy() = x.vx() * std::sin(x.theta()) + (std::cos(x.theta()) * antenna_offset_x * x.vr()) -
                               (std::sin(x.theta()) * antenna_offset_y * x.vr());
        ...
        void updateJacobians(const S &x) {
            ...
            this->H(M::VX, S::THETA) = -x.vx() * std::sin(x.theta()) - antenna_offset_x * x.vr() * std::cos(x.theta()) +
                                       std::sin(x.theta()) * antenna_offset_y * x.vr();
            this->H(M::VY, S::THETA) = x.vx() * std::cos(x.theta()) - antenna_offset_x * x.vr() * std::sin(x.theta()) -
                                       std::cos(x.theta()) * antenna_offset_y * x.vr();
        }
